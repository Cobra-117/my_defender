...
int is_upgrade(Index_t *index)
{
    ...
    index->ui_button_skills.click_pos_upgrade.y + index->ui_button_skills.
    click_size_upgrade.y && index->ui.coord_mouse_y >= index->ui_button_skills.
    click_pos_upgrade.y && index->what_is_open == 3) {
        index->ui_which_turret = 11;
        printf("UPGRADE\n");
    }
}